...
const AGENTS = [
  { name: "Hammad Ahmed", phone: "923325500377" },
...
function buildWhatsAppUrl(
  ...
  name: string,
  tripType: string,
  legs: { fromLabel: string; toLabel: string; dateLabel: string }[],
  retDate: string,
  pax: string,
  cabin: string,
  contact: string,
  ...
  const tripLabel =
    tripType === "round" ? "Round Trip" : tripType === "one" ? "One Way" : "Multi-City";
  const lines = [
...
export function FlightSearchPortal() {
  ...
  const addLeg = () => {
    if (extraLegs.length < 3) setExtraLegs([...extraLegs, { from: "", to: "", date: undefined }]);
  };
  const removeLeg = (i: number) => setExtraLegs(extraLegs.filter((_, idx) => idx !== i));
  const updateLeg = (i: number, patch: Partial<Leg>) =>
    setExtraLegs(extraLegs.map((l, idx) => (idx === i ? { ...l, ...patch } : l)));
  ...
  const getAllLegs = () => {
    const first = {
      fromLabel: codeToLabel(fromCode),
      toLabel: codeToLabel(toCode),
      dateLabel: date ? format(date, "PPP") : "",
    };
    if (tripType !== "multi") return [first];
    return [
      first,
      ...extraLegs.map((l) => ({
        fromLabel: codeToLabel(l.from),
        toLabel: codeToLabel(l.to),
        dateLabel: l.date ? format(l.date, "PPP") : "Flexible",
      })),
    ];
  };

  const getAgentUrl = (agent: (typeof AGENTS)[0]) =>
    buildWhatsAppUrl(
      agent.phone,
      agent.name,
      tripType,
  ...
  const canSearch =
    fromCode && toCode && date && (tripType !== "multi" || extraLegs.every((l) => l.from && l.to));
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-10 shadow-2xl flex flex-col items-center gap-5 min-w-[280px] animate-in zoom-in-95 duration-300">
            ...
            <p className="text-sm text-muted-foreground text-center">
              Searching across 300+ airlines for you
            </p>
          </div>
        </div>
      )}
      ...
      {showAgentPicker && (
        ...
        >
          <div
            className="bg-white rounded-3xl p-8 sm:p-10 shadow-2xl flex flex-col items-center gap-6 w-[90vw] max-w-[420px] animate-in zoom-in-95 duration-300"
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Choose an agent to get your personalized quote on WhatsApp
              </p>
            </div>
              ...
              {AGENTS.map((agent) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-bold text-slate-800 group-hover:text-[#25D366] transition-colors">
                      {agent.name}
                    </p>
      ...
      <div className="w-full bg-white dark:bg-card rounded-2xl shadow-elevated p-4 sm:p-6 mb-12 border border-border mt-8 sm:-mt-16 relative z-20">
          ...
          <div className="flex items-center rounded-full border border-blue-200 overflow-hidden bg-white">
            {(["round", "one", "multi"] as const).map((t, i) => (
              <button
                ...
                className={cn(
                  "px-4 py-1.5 font-medium transition-colors outline-none",
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button
                ...
                className="flex items-center gap-1.5 hover:text-foreground transition-colors outline-none text-muted-foreground font-medium"
              >
                <Armchair size={16} className="text-blue-500" /> {cabinClass}{" "}
            ...
            <PopoverContent className="w-44 p-1">
              {["Economy", "Premium Economy", "Business", "First"].map((c) => (
                <div
                  ...
                  className="p-2.5 hover:bg-muted cursor-pointer text-sm rounded-lg font-medium"
                >
                  {c}
                </div>
              ))}
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button
                ...
                className="flex items-center gap-1.5 hover:text-foreground transition-colors outline-none text-muted-foreground font-medium"
              >
                <Users size={16} className="text-blue-500" /> {totalPax} Traveller
                {totalPax > 1 ? "s" : ""} <ChevronDown size={14} className="opacity-50" />
              </button>
            </PopoverTrigger>
            ...
            <Popover open={fromOpen} onOpenChange={setFromOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  variant="outline"
                  role="combobox"
                  className="justify-start w-full sm:rounded-r-none h-14 pl-12 text-base font-normal border-border bg-white shadow-none hover:bg-muted/50 hover:text-foreground"
                >
                  <PlaneTakeoff
                  ...
                  />
                  {fromCode ? (
                    codeToLabel(fromCode)
                  ) : (
                    <span className="text-muted-foreground">From Where</span>
                  )}
                </Button>
              </PopoverTrigger>
            ...
            <Popover open={toOpen} onOpenChange={setToOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  variant="outline"
                  role="combobox"
                  className="justify-start w-full sm:rounded-l-none border-t-0 sm:border-t sm:border-l-0 h-14 pl-12 sm:pl-10 text-base font-normal border-border bg-white shadow-none hover:bg-muted/50 hover:text-foreground"
                >
                  <PlaneLanding
                    className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-500"
                    size={20}
                  ...
                  {toCode ? (
                    ...
                    <span className="text-muted-foreground">To Where</span>
                  )}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-[300px] p-0" align="start">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className={cn(
                  "h-14 px-4 text-base bg-white border-border hover:bg-muted/50 shadow-none justify-start text-left font-normal lg:w-[160px]",
        ...
        {tripType === "multi" && (
          <div className="mt-4 space-y-3">
            {extraLegs.map((leg, i) => (
              <div
                key={i}
                className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center p-3 rounded-xl bg-blue-50/50 border border-blue-100"
              >
                <span className="text-xs font-bold text-blue-600 shrink-0 self-center">
                  Leg {i + 2}
                </span>
                {/* From */}
                <AirportPicker
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className={cn(
                        "h-12 px-4 text-sm bg-white border-border hover:bg-muted/50 shadow-none justify-start text-left font-normal min-w-[140px]",
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-10 w-10 shrink-0 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-full"
                  onClick={() => removeLeg(i)}
                >
                  <X size={18} />
                </Button>
            ...
            {extraLegs.length < 3 && (
              <button
                onClick={addLeg}
                className="flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700 transition-colors pl-1 outline-none"
              >
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-4">
          <div className="relative flex-1 sm:max-w-[220px]">
            <label htmlFor="flight-contact" className="sr-only">
              Contact Number
            </label>
            ...
            <Input
              ...
              className="pl-11 h-14 text-base bg-white border-border shadow-none"
            />
          </div>
          <Button
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          role="combobox"
          className="justify-start h-12 pl-10 text-sm font-normal border-border bg-white shadow-none hover:bg-muted/50 hover:text-foreground flex-1 min-w-[140px]"
        >
          <Icon className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-500" size={16} />
          {label ? (
            `${label.city} (${label.code})`
          ) : (
