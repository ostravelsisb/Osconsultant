    ...
    <>
      ...
      <section className="container-px mx-auto max-w-5xl py-20">
        ...
        <div className="space-y-6 text-center max-w-3xl mx-auto mb-20">
          <h2 className="text-3xl font-bold md:text-4xl">Expert Processing for {destination.name} {visa.name}s</h2>
          <p className="text-muted-foreground leading-relaxed text-lg">
            As the top visa consultancy in Islamabad, OS Consultants ensures your {visa.name.toLowerCase()} application for {destination.name} is meticulously prepared. We guide you through the entire process, minimizing errors and maximizing your chances of approval.
          </p>
        </div>
            ...
            <ul className="space-y-4">
              {visa.requirements.map((req, idx) => (
                <li key={idx} className="flex items-start gap-4">
                  ...
                  <span className="text-muted-foreground leading-relaxed font-medium text-sm">{req}</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      <section className="bg-secondary/30 py-20 border-t border-border">
        <div className="container-px mx-auto max-w-7xl">
          <div className="grid gap-12 lg:grid-cols-2 items-center">
            <div className="space-y-6">
              <h2 className="text-3xl font-bold md:text-4xl">Ready to apply for your {visa.name}?</h2>
              <p className="text-muted-foreground leading-relaxed text-lg">
                Don't risk your application with incomplete documents. Let the best {destination.name} visa consultants in Islamabad handle your case.
              </p>
              <ul className="space-y-3 font-medium">
                <li className="flex items-center gap-3"><CheckCircle2 className="text-primary" size={18}/> Form Filling & Error Checking</li>
                <li className="flex items-center gap-3"><CheckCircle2 className="text-primary" size={18}/> Document Translation & Attestation Guidance</li>
                <li className="flex items-center gap-3"><CheckCircle2 className="text-primary" size={18}/> Appointment Scheduling & Mock Interviews</li>
              </ul>
            ...
            <div className="bg-card rounded-[2rem] border border-border p-8 shadow-elevated relative overflow-hidden">
               ...
               <p className="text-sm text-muted-foreground mb-6">Leave your contact info and our experts will reach out to discuss your {visa.name.toLowerCase()} for {destination.name}.</p>
               <ContactForm />
            </div>
          </div>
        </div>
